...
ny_noaa %>% 
  ...
  count(snow) %>% 
	arrange(desc(n))
```
The NY NOAA dataset is a large rectangle of `r nrow(ny_noaa)` weather observations in NY state, structured by `r distinct(ny_noaa, id)` weather stations at which the data were collected, in conjunction with the date of that observation. Then, there is the total precipitation (in tenths of millimeters), specifically snow characteristics, and the temperature minimum and maximum (in celsius). 

The most commonly observed snowfall values is 0 (since most days in these locations, it doesn't snow!). Then, unfortunately, there are a lot of missing values for snowfall (potentially measurement error, potentially a null value), and then various millimeter values. 
